  ...
  */
	...
	var accordion = $.widget("ui.accordion", {
		...
		_eventHandler: function _eventHandler(event) {
			...
			clickedIsActive && !options.collapsible || this._trigger("beforeActivate", event, eventData) === false) {
				return;
			}
				...
				for (col = 0; col < numMonths[1]; col++) {
					...
					for (dRow = 0; dRow < numRows; dRow++) {
						...
						for (dow = 0; dow < 7; dow++) {
							...
							tbody += "<td class='" + ((dow + firstDay + 6) % 7 >= 5 ? " ui-datepicker-week-end" : "") + (otherMonth ? " ui-datepicker-other-month" : "") + (printDate.getTime() === selectedDate.getTime() && drawMonth === inst.selectedMonth && inst._keyEvent || defaultDate.getTime() === printDate.getTime() && defaultDate.getTime() === selectedDate.getTime() ?
  ...
  */
	...
	var tabs = $.widget("ui.tabs", {
		...
		_eventHandler: function _eventHandler(event) {
			...
			this.running || clickedIsActive && !options.collapsible || this._trigger("beforeActivate", event, eventData) === false) {
				return;
			}
...
(function ($) {
	...
	var MetisMenu = function MetisMenu(element, options) {
		this.$element = $(element);
	...
	MetisMenu.prototype.show = function (el) {
		...
		this.transitioning = 1;
		var complete = function complete() {
			$this.removeClass("collapsing").addClass("collapse in").height("");
			this.transitioning = 0;
		};
		if (!$transition) {
			return complete.call(this);
		}
		$this.one("mmTransitionEnd", $.proxy(complete, this)).emulateTransitionEnd(MetisMenu.TRANSITION_DURATION).height($this[0].scrollHeight);
	};
	...
	MetisMenu.prototype.hide = function (el) {
		...
		$this.addClass("collapsing").removeClass("collapse").removeClass("in");

		this.transitioning = 1;

		var complete = function complete() {
			this.transitioning = 0;
			$this.removeClass("collapsing").addClass("collapse");
		};

		if (!$transition) {
			return complete.call(this);
		}
		$this.height(0).one("mmTransitionEnd", $.proxy(complete, this)).emulateTransitionEnd(MetisMenu.TRANSITION_DURATION);
	};
...
/* options, element */ /* event */ /* event */ /* event */ /* event */ /* event */
...
// The offsetParent's offset without borders (offset + border)
// The offsetParent's offset without borders (offset + border)
/* valueToWrite */ // Top edge touching
// Bottom edge touching
// Left edge touching
// Right edge touching

// wrapped to prevent IE from throwing errors on "invalid" values like 'auto' or 'inherit'
/* effect, options, speed, callback */ // The offsetParent's offset without borders (offset + border)
// The offsetParent's offset without borders (offset + border)
// The offsetParent's offset without borders (offset + border)
// The offsetParent's offset without borders (offset + border)
