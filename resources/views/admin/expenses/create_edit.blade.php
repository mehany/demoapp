...
<form class="form-horizontal" enctype="multipart/form-data"
	method="post"
	action="@if(isset($expenses)){{ URL::to('admin/expenses/'.$expense->id.'/edit') }}
	        @else{{ URL::to('admin/expenses/create') }}@endif"
	autocomplete="off">
	<!-- CSRF Token -->
		...
		<div class="tab-pane active" id="tab-general">
			<div class="tab-pane active" id="tab-general">
				...
				<div
					...
					<div class="col-md-12">
						<label class="control-label" for="title"> {{
							...
							value="{{{ Input::old('title', isset($expenses) ? $expenses->title : null) }}}" />
						{!!$errors->first('title', '<label class="control-label">:message</label>')!!}
					</div>
				</div>
				<div
					class="form-group {{{ $errors->has('expense_category_id') ? 'error' : '' }}}">
					<div class="col-md-12">
						<label class="control-label" for="expense_category_id">{{
							trans("admin/expenses.category") }}</label> <select
							style="width: 100%" name="expense_category_id" id="expense_category_id"
							class="form-control"> @foreach($expenseCategories as $item)
							<option value="{{$item->id}}"
								@if(!empty($expenseCategory))
                                        @if($item->id==$expenseCategory)
								selected="selected" @endif @endif >{{$item->title}}</option>
							@endforeach
						</select>
						{!!$errors->first('expense_category_id', '<label class="control-label">:message</label>')!!}
					</div>
				</div>
				
				<div
					class="form-group {{{ $errors->has('expense_sub_category_id') ? 'error' : '' }}}">
					<div class="col-md-12">
						<label class="control-label" for="expense_sub_category_id">{{
							trans("admin/expenses.category") }}</label> <select
							style="width: 100%" name="expense_sub_category_id" id="expense_sub_category_id"
							class="form-control"> @foreach($expenseSubCategories as $item)
							<option value="{{$item->id}}"
								@if(!empty($expenseSubCategory))
                                        @if($item->id==$expenseSubCategory)
								selected="selected" @endif @endif >{{$item->title}}</option>
							@endforeach
						</select>
					</div>
				...
				<div
					...
					<div class="col-md-12">
						<label class="control-label" for="content">{{
							trans("admin/expenses.content") }}</label>
						<textarea class="form-control full-width wysihtml5" name="content"
							value="content" rows="10">{{{ Input::old('content', isset($expense) ? $expense->content : null) }}}</textarea>
				...
				<div
					...
					<div class="col-md-12">
						<label class="control-label" for="title">{{
							trans("admin/expenses.source") }}</label> <input
							class="form-control" type="text" name="source" id="source"
							value="{{{ Input::old('source', isset($expenses) ? $expenses->source : null) }}}" />
				...
				<div
					...
					<div class="col-lg-12">
						<label class="control-label" for="picture">{{
							trans("admin/expenses.picture") }}</label> <input name="picture"
